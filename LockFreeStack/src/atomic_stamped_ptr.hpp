...
class AtomicStampedPtr {
 ...
 public:
  ...
  bool CompareExchangeWeak(StampedPtr<T>& expected, StampedPtr<T> desired) noexcept {
  ...
  bool CompareExchangeStrong(StampedPtr<T>& expected, StampedPtr<T> desired) noexcept {
    PackedPtr expected_packed = Pack(expected);
    bool succeeded =
        packed_ptr_.compare_exchange_strong(expected_packed, Pack(desired));
    if (!succeeded) {
      expected = Unpack(expected_packed);
    }
    return succeeded;
